...
router.post('/redeem', requireAuth, async (req, res) => {
  ...
  const promo = await db.prepare('SELECT * FROM promo_codes WHERE code = ?').get(String(code).trim().toUpperCase());
  ...
  await db.prepare('INSERT INTO promo_redemptions (promo_code_id, user_id) VALUES (?, ?)').run(promo.id, req.user.id);
  await db.prepare('UPDATE promo_codes SET uses_count = uses_count + 1 WHERE id = ?').run(promo.id);
  ...
  await recordTransaction(req.user.id, 'promo', promo.credits, newBalance, `Código promocional ${promo.code}`);
